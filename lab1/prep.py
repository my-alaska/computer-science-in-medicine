...
class Inka:
    ...
    def load_video_data():
        ...
        if not is_pickle:
            ...
            print(
                "loaded frames from video: {}  ({:.2f} s.)".format(
                    len(frames), (time.time() - t_start)
                )
            )
            ...
            print("pickle loaded")
            print(
                "loaded frames from pickle: {}  ({:.2f} s.)".format(
                    len(frames), (time.time() - t_start)
                )
            )
            print(
                "loaded filtered frames from pickle: {}  ({:.2f} s.)".format(
                    len(frames_filtered), (time.time() - t_start)
                )
            )
    ...
    def anim_process_frame(self, i):
        ...
        frame_diff_median = np.abs(frame_filtered - self.frame_median)
        frame_diff_median = cv2.normalize(
            frame_diff_median,
            None,
            alpha=0,
            beta=255,
        ...
        frame_diff_median_rgb = cv2.applyColorMap(
            frame_diff_median.astype(np.uint8), cv2.COLORMAP_AUTUMN
        )
        ...
        frame_rgb = cv2.rectangle(
            frame_rgb, roi_start, roi_end, self.roi_color, self.roi_thickness
        )
        ...
        if self.use3d:
            ...
            self.surface_plot = self.surface_ax.plot_surface(
                self.xx, self.yy, zz, linewidth=0, antialiased=False, cmap=cm.plasma
            )
    ...
    def display_spectro(y, sr):
        ...
        img = librosa.display.specshow(
            D, y_axis="linear", x_axis="time", sr=sr, ax=ax[0]
        )
    ...
    def display_video_and_audio(self):
        ...
        self.line_audio_proc = plt.axvline(
            x=0, ymin=0, ymax=1, color="r", linewidth="1"
        )
